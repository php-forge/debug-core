...
final class ProfileTimingsTest extends TestCase
{
    public function testCalculatePairsNestedIdenticalTokensWithoutLosingOuterFrame(): void
    {
        ...
        self::assertCount(2, $timings, 'Both nested frames with the same token must survive pairing.');
        ...
        self::assertSame(0, $outer['level'], 'Outer repeated token must retain nesting level zero.');
        self::assertSame(1, $inner['level'], 'Inner repeated token must retain nesting level one.');
        self::assertEqualsWithDelta(0.3, $outer['duration'], PHP_FLOAT_EPSILON, 'Outer duration must pair last.');
        self::assertEqualsWithDelta(0.1, $inner['duration'], PHP_FLOAT_EPSILON, 'Inner duration must pair first.');
        self::assertSame(40, $outer['memoryDiff'], 'Outer memory delta must use the outer begin frame.');
        self::assertSame(10, $inner['memoryDiff'], 'Inner memory delta must use the inner begin frame.');
    }
}
