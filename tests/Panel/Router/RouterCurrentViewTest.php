...
final class RouterCurrentViewTest extends TestCase
{
    public function testFromSnapshotProjectsTheHydratedSnapshot(): void
    {
        ...
        self::assertSame(
            1,
            $view->count,
            'Log count must reflect the entries.',
        );
        self::assertTrue(
            $view->hasMatch,
            'A matching entry must flag the view.',
        );
        self::assertCount(
            1,
            $view->logs,
            'Log rows must project.',
        );
    }
    ...
    public function testFromSnapshotReturnsEmptyViewForNull(): void
    {
        ...
        self::assertSame(
            '',
            $view->action,
            'Empty view must carry no action.',
        );
        self::assertSame(
            '',
            $view->route,
            'Empty view must carry no route.',
        );
        self::assertNull(
            $view->message,
            'Empty view must carry no message.',
        );
        self::assertSame(
            0,
            $view->count,
            'Empty view must count zero rules.',
        );
        self::assertFalse(
            $view->hasMatch,
            'Empty view must report no match.',
        );
        self::assertSame(
            [],
            $view->logs,
            'Empty view must carry no logs.',
        );
    }
}
