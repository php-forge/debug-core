...
final class SidebarRendererTest extends TestCase
{
    public function testRenderEmitsAriaCurrentOnActiveNavLink(): void
    {
        ...
        self::assertStringContainsString('aria-current="page"', $html, 'Active nav entry must carry aria-current.');
        self::assertStringContainsString('is-active', $html, 'Active nav entry must carry the is-active modifier.');
        self::assertSame(1, substr_count($html, 'is-active'), 'Only the active entry carries the modifier.');
    ...
    public function testRenderEmitsCursorButtonsWhenSnapshotIsCursor(): void
    {
        ...
        self::assertStringContainsString('<button', $html, 'Cursor mode must use buttons instead of anchors.');
    }
