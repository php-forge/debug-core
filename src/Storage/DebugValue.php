...
{
    /**
     * Defines the required fields for each encoded entry.
     *
     * @var list<string>
     */
    ...
    /**
     * Limits the maximum nesting depth captured or hydrated.
     */
    private const int MAX_DEPTH = 10;
    /**
     * Limits the maximum number of tagged values processed during capture.
     */
    private const int MAX_NODES = 10000;
    /**
     * Maps each tagged value type to its required fields.
     *
    ...
    private const array SHAPES = [
        ...
        'int' => ['type', 'value'],
        'float' => ['type', 'value'],
        'special-float' => ['type', 'value'],
        'string' => ['type', 'value'],
        'binary' => ['type', 'encoding', 'data'],
        'array' => ['type', 'entries'],
        'object' => ['type', 'value', 'entries', 'class'],
        'resource' => ['type', 'resourceType'],
        'truncated' => ['type', 'value', 'reason'],
        'recursion' => ['type', 'value', 'reason'],
        'unsupported' => ['type', 'value', 'reason'],
    ];
    ...
    {
        $entry = Payload::object($value, $path)->shape(self::ENTRY_SHAPE);

        return [
            'keyType' => $entry->raw('keyType'),
            'key' => $entry->raw('key'),
     ...
     *
     * @return Payload Validated tagged-value reader.
     */
    private static function taggedObject(mixed $value, string $path, string &$type): Payload
    {
        $payload = Payload::object($value, $path);
        $rawType = $payload->raw('type');
        ...
        return $payload->shape(self::SHAPES[$type]);
    }
}
