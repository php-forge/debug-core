...
{
    /**
     * Matches the `<body>` content of a complete {@see phpinfo()} HTML document.
     */
    private const string BODY_PATTERN = '%^.*<body>(.*)</body>.*$%ms';
    /**
    ...
    {
        // `ob_start`, `phpinfo`, `ob_get_clean`, and `ini_get` stay unimported so `MockerExtension` can swap them.
        ob_start();
        phpinfo();

        $output = ob_get_clean();

        return self::fromOutput(
            body: $output === false ? '' : preg_replace(self::BODY_PATTERN, '$1', $output) ?? '',
