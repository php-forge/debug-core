...
test("a collapse control toggles the region named by data-target", async () => {
  const page = await bootDebugPage({
    body:
      '<button type="button" data-yii-debug-toggle="collapse" ' +
      'data-target="#trace"><span>Trace</span></button>' +
      '<div id="trace" class="is-open">frames</div>',
  });
  const control = page.document.querySelector(
    '[data-yii-debug-toggle="collapse"]',
  );
  const target = page.document.getElementById("trace");

  fire(control.querySelector("span"), "click");

  assert.equal(target.classList.contains("is-open"), false, "Must collapse.");
  assert.equal(control.getAttribute("aria-expanded"), "false", "State down.");

  fire(control, "click");

  assert.equal(target.classList.contains("is-open"), true, "Must expand.");
  assert.equal(control.getAttribute("aria-expanded"), "true", "State is up.");
});

test("a collapse control falls back to its href", async () => {
  const page = await bootDebugPage({
    body:
      '<a data-yii-debug-toggle="collapse" href="#trace">Trace</a>' +
      '<div id="trace">frames</div>',
  });

  fire(page.document.querySelector("a"), "click");

  assert.equal(
    page.document.getElementById("trace").classList.contains("is-open"),
    true,
    "Fragment href must name the region.",
  );
});

test("a collapse control without a target is inert", async () => {
  const page = await bootDebugPage({
    body: '<button type="button" data-yii-debug-toggle="collapse">Trace</button>',
  });
  const control = page.document.querySelector(
    '[data-yii-debug-toggle="collapse"]',
  );
  ...
  assert.equal(
    ...
    null,
    "Nothing to expand means no state.",
  );
});

test("a collapse control pointing at a missing region is inert", async () => {
  const page = await bootDebugPage({
    body:
      '<button type="button" data-yii-debug-toggle="collapse" ' +
      'data-target="#gone">Trace</button>',
  });
  const control = page.document.querySelector(
    '[data-yii-debug-toggle="collapse"]',
  );
  ...
  assert.equal(
    control.getAttribute("aria-expanded"),
    null,
    "A dangling target must leave the control alone.",
  );
});
